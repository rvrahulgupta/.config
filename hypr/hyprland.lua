...
local menu        = "noctalia msg panel-toggle menu"
...
hl.config({
    general = {
        ...
        col = {
            active_border   = { colors = {"rgba(c0c0c0ff)", "rgba(c0c0c0ff)"}, angle = 45 },
...
local closeWindowBind = hl.bind("ALT + C", hl.dsp.window.close())
local altMod = "ALT"
...
hl.bind(mainMod .. " + L", hl.dsp.exec_cmd("noctalia msg panel-toggle launcher"))
...
hl.bind("CTRL+ALT+O", hl.dsp.exec_cmd("command -v hyprshutdown >/dev/null 2>&1 && hyprshutdown || hyprctl dispatch 'hl.dsp.exit()'"))
hl.bind("CTRL+ALT+U", hl.dsp.exec_cmd("noctalia msg session lock-and-suspend"))
hl.bind("CTRL+ALT+L", hl.dsp.exec_cmd("noctalia msg session lock"))
hl.bind("CTRL+ALT+S", hl.dsp.exec_cmd("poweroff"))
hl.bind("CTRL+ALT+R", hl.dsp.exec_cmd("systemctl restart"))
...
hl.bind("ALT + Z", hl.dsp.exec_cmd("noctalia msg bar-toggle"))
hl.bind(altMod .. " + F", hl.dsp.window.fullscreen({ mode = "maximized" }))
hl.bind(altMod .. " + SHIFT + F", hl.dsp.window.fullscreen({ mode = "fullscreen" }))
...
-- Scroll through existing workspaces on the current monitor (with looping)
hl.bind(mainMod .. " + Right", hl.dsp.focus({ workspace = "m+1" }))
hl.bind(mainMod .. " + Left",  hl.dsp.focus({ workspace = "m-1" }))

-- Optional: Move the active window to the next/prev existing workspace
...
hl.window_rule({
    ...
    move  = "40 monitor_h-180",
    float = false,
})

-- Force GNOME's file picker portal to float and center nicely
hl.window_rule({
    match = { 
        class = "org.gnome.Nautilus", 
        title = "Open Document|Save File|Save As" 
    },
    float = true,
    center = true
})
